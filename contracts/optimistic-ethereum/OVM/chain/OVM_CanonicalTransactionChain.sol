...
contract OVM_CanonicalTransactionChain is iOVM_CanonicalTransactionChain, Lib_AddressResolver {
   ...
   /**
    ...
    {
        (uint40 totalElements,,,) = _getBatchExtraData(_chainId);
        return uint256(totalElements);
    }
    ...
    {
        return batches().length(_chainId);
    }

    /**
     * @inheritdoc iOVM_CanonicalTransactionChain
     */
    function getNextQueueIndex(uint256 _chainId)
        override
        public
        view
        returns (
    ...
    {
        (,uint40 nextQueueIndex,,) = _getBatchExtraData(_chainId);
        return nextQueueIndex;
    }
     ...
     * @inheritdoc iOVM_CanonicalTransactionChain
     */
    function getLastTimestamp(uint256 _chainId)
        override
        public
        view
        returns (
    ...
    {
        (,,uint40 lastTimestamp,) = _getBatchExtraData(_chainId);
        return lastTimestamp;
    }
     ...
     * @inheritdoc iOVM_CanonicalTransactionChain
     */
    function getLastBlockNumber(uint256 _chainId)
        override
        public
        view
        returns (
    ...
    {
        (,,,uint40 lastBlockNumber) = _getBatchExtraData(_chainId);
        return lastBlockNumber;
    }
        ...
        )
    {
        ...
        uint40 trueIndex = uint40(_index * 2);
        bytes32 queueRoot = queue.get(_chainId,trueIndex);
        bytes32 timestampAndBlockNumber = queue.get(_chainId,trueIndex + 1);
    ...
    {
        return getQueueLength(_chainId) - getNextQueueIndex(_chainId);
    }
   ...
   /**
        ...
        )
    {
        ...
        // per insertion, so to get the real queue length we need
        // to divide by 2. See the usage of `push2(..)`.
        return uint40(queue().length(_chainId) / 2);
    }
        ...
        public
    {
        ...
        _numQueuedTransactions = Lib_Math.min(_numQueuedTransactions, getNumPendingQueueElements(_chainId));
        require(
            _numQueuedTransactions > 0,
            "Must append more than zero transactions."
        );
        ...
        bytes32[] memory leaves = new bytes32[](_numQueuedTransactions);
        uint40 nextQueueIndex = getNextQueueIndex(_chainId);

        for (uint256 i = 0; i < _numQueuedTransactions; i++) {
            if (msg.sender != resolve("OVM_Sequencer")) {
                Lib_OVMCodec.QueueElement memory el = getQueueElement(_chainId,nextQueueIndex);
                require(
                    el.timestamp + forceInclusionPeriodSeconds < block.timestamp,
                    "Queue transactions cannot be submitted during the sequencer inclusion period."
                );
            }
            leaves[i] = _getQueueLeafHash(_chainId,nextQueueIndex);
            nextQueueIndex++;
        }

        Lib_OVMCodec.QueueElement memory lastElement = getQueueElement(_chainId,nextQueueIndex - 1);
        ...
        public
    {
        ...
        require(
            shouldStartAtElement == getTotalElements(_chainId),
            "Actual batch start index does not match expected start index."
        );
        ...
        // Get queue length for future comparison/
        uint40 queueLength = getQueueLength(_chainId);
        ...
        for (uint32 i = 0; i < numContexts; i++) {
            BatchContext memory nextContext = _getBatchContext(_chainId,i);
            if (i == 0) {
                _validateFirstBatchContext(_chainId,nextContext);
            }
            _validateNextBatchContext(_chainId,curContext, nextContext, nextQueueIndex);

            curContext = nextContext;

            uint256 myChain=_chainId;
            for (uint32 j = 0; j < curContext.numSequencedTransactions; j++) {
                uint256 txDataLength;
                assembly {
                    txDataLength := shr(232, calldataload(nextTransactionPtr))
                ...
                leaves[leafIndex] = _getSequencerLeafHash(myChain, curContext, nextTransactionPtr, txDataLength);
                nextTransactionPtr += uint40(TX_DATA_HEADER_SIZE + txDataLength);
                numSequencerTransactions++;
                leafIndex++;
            }
            ...
            for (uint32 j = 0; j < curContext.numSubsequentQueueTransactions; j++) {
                require(nextQueueIndex < queueLength, "Not enough queued transactions to append.");
                leaves[leafIndex] = _getQueueLeafHash(myChain,nextQueueIndex);
                nextQueueIndex++;
                leafIndex++;
            }
        }

        _validateFinalBatchContext(_chainId,curContext);
        ...
        if (curContext.numSubsequentQueueTransactions == 0) {
            ...
            // The last element is a queue tx, therefore pull timestamp and block number from the queue element.
            uint256 myChain2=_chainId;
            Lib_OVMCodec.QueueElement memory lastElement = getQueueElement(myChain2, nextQueueIndex - 1);
            timestamp = lastElement.timestamp;
            blockNumber = lastElement.blockNumber;
        }
    ...
    {
        uint256 contextPtr = 32+15 + _index * BATCH_CONTEXT_SIZE;
        uint256 numSequencedTransactions;
        uint256 numSubsequentQueueTransactions;
        uint256 ctxTimestamp;
        uint256 ctxBlockNumber;
    ...
    {
        bytes27 extraData = batches().getGlobalMetadata(_chainId);
        ...
        internal
    {
        ...
        batches().push(_chainId,batchHeaderHash, latestBatchContext);
    }
    ...
    {
        // If there are existing elements, this batch must come later.
        if (getTotalElements(_chainId) > 0) {
            (,, uint40 lastTimestamp, uint40 lastBlockNumber) = _getBatchExtraData(_chainId);
            require(_firstContext.blockNumber >= lastBlockNumber, "Context block number is lower than last submitted.");
            require(_firstContext.timestamp >= lastTimestamp, "Context timestamp is lower than last submitted.");
        }
        // Sequencer cannot submit contexts which are more than the force inclusion period old.
        ...
        view
    {
        ...
        // If there are some queue elements pending:
        if (getQueueLength(_chainId) - _nextQueueIndex > 0) {
            Lib_OVMCodec.QueueElement memory nextQueueElement = getQueueElement(_chainId,_nextQueueIndex);
        ...
        )
    {
        OVM_ExecutionManager ovmExecutionManager = OVM_ExecutionManager(resolve("OVM_ExecutionManager"));
        uint256 gasLimit = ovmExecutionManager.getMaxTransactionGasLimit();
        bytes32 leafHash = _getSequencerLeafHash(_chainId,_txChainElement);
    ...
    {
        bytes32 leafHash = _getQueueLeafHash(_chainId,_queueIndex);
        ...
        Lib_OVMCodec.QueueElement memory el = getQueueElement(_chainId,_queueIndex);
        require(
            el.queueRoot      == transactionHash
            && el.timestamp   == _transaction.timestamp
            && el.blockNumber == _transaction.blockNumber,
    ...
    {
        require(
            Lib_OVMCodec.hashBatchHeader(_batchHeader) == batches().get(_chainId,uint32(_batchHeader.batchIndex)),
            "Invalid batch header."
        );
