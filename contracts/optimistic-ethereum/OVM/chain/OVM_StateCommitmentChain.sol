...
contract OVM_StateCommitmentChain is iOVM_StateCommitmentChain, Lib_AddressResolver {
    ...
    uint256 public DEFAULT_CHAINID = 0;
    ...
    {
        (, uint40 lastSequencerTimestamp) = _getBatchExtraData(_chainId);
        return uint256(lastSequencerTimestamp);
    }
        ...
        public
    {
        ...
        // publication of batches by some other user.
        require(
            _shouldStartAtElement == getTotalElements(_chainId),
            "Actual batch start index does not match expected start index."
        );
        ...
        require(
            getTotalElements(_chainId) + _batch.length <= iOVM_CanonicalTransactionChain(resolve("OVM_CanonicalTransactionChain")).getTotalElements(_chainId),
            "Number of state roots cannot exceed the number of canonical transactions."
        );
        ...
        public
    {
        ...
        require(
            _isValidBatchHeader(_chainId,_batchHeader),
            "Invalid batch header."
        );

        require(
            insideFraudProofWindow(_chainId,_batchHeader),
            "State batches can only be deleted within the fraud proof window."
        );

        _deleteBatch(_chainId,_batchHeader);
    }
    ...
    {
        address sequencer = resolve("OVM_Sequencer");
        (uint40 totalElements, uint40 lastSequencerTimestamp) = _getBatchExtraData(_chainId);
        ...
        Lib_OVMCodec.ChainBatchHeader memory batchHeader = Lib_OVMCodec.ChainBatchHeader({
            batchIndex: getTotalBatches(_chainId),
            batchRoot: Lib_MerkleTree.getMerkleRoot(_batch),
            batchSize: _batch.length,
            prevTotalElements: totalElements,
            extraData: _extraData
    ...
    {
        require(
            _batchHeader.batchIndex < batches().length(_chainId),
            "Invalid batch index."
        );
    ...
    {
        return Lib_OVMCodec.hashBatchHeader(_batchHeader) == batches().get(_chainId,_batchHeader.batchIndex);
    }
}
